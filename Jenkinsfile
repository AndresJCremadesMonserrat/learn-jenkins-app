pipeline {
    ...
    stages {
        ...
        stage('Deploy staging') {
            ...
            environment {
                CI_ENVIRONMENT_URL = 'STAGING _URL_TO_BE_SET'
            }
            ...
            steps {
                sh '''
                    ...
                    CI_ENVIRONMENT_URL=$(node_modules/.bin/node-jq -r '.deploy_url' deploy-output.json) #similar to script
                    npx playwright test  --reporter=html
                '''
            }
        ...
        stage('Approval') {
            steps {
                timeout(time: 15, unit: 'MINUTES') {
                    input message: 'Do you wish to deploy to production?', ok: 'Yes, I am sure!', submitter: 'adminandres'
                }
            }
        }
        ...
        stage('Deploy production') {
            ...
            steps {
                sh '''
                    ...
                    echo "Deploying to production. Site ID: $NETLIFY_SITE_ID"
                    node_modules/.bin/netlify status
                    node_modules/.bin/netlify deploy --dir=build --prod #this command may need some seconds to deploy
                    npx playwright test  --reporter=html
                '''
            }
